...
### 5. ⚡ Playwright 스크래퍼 최적화 (6.9배 빨라짐!)
- 브라우저 재사용, 리소스 완전 차단, 페이지 풀 적용
- 벤치마크 결과 (5개 언론사, 456개 기사):
  - **기존**: 85.56초 (기사당 187.6ms)
  - **최적화**: 12.48초 (기사당 27.4ms)
  - **속도 향상**: **6.9배 빠름!**
- 새 스크래퍼: `scraper_optimized.py`
- 주의: httpx는 사용 불가 (페이지가 JS 렌더링이기 때문)

### 6. 🔍 Keyword Filter (Feature 1)
- Added sidebar keyword input for filtering articles.
- Supports multiple keywords separated by comma.
- Filters both title and subtitle content.

### 7. 🤖 AI 1-Line Summary (Feature 2)
- Added `generate_one_line_summary()` function in `analysis.py`.
- Uses Gemini Flash model for fast, low-cost summarization.

### 8. 📁 Folder & Tag System (Feature 3)
- Scraps can now be organized into folders.
- Added folder selection dropdown in Scrapbook.
- Added "Add Folder" button to create new folders.
- Tags support added (stored per scrap).

### 9. 📝 Markdown Export (Feature 6)
- Added "마크다운 내보내기" button in Scrapbook.
- Exports filtered scraps to `.md` file with proper formatting.

### 10. 📆 Date Logic & Sunday Report
- Changed default date from yesterday to today.
- Added Sunday detection: when opening the app on Sunday, shows "Weekly Report" prompt.
- Since newspapers don't publish on Sundays, users can generate AI weekly report or select a different date.

### 11. 📚 Section-Based Pagination
- Automatically groups pages by section (A, B, E, S, etc.)
- Splits each section by **page number range** (1-10, 11-20, 21-30, ...)
- Displays navigation buttons: `A1-10`, `A11-20`, `B1-10`, etc.
- Selected button highlighted in blue
- Handles sparse page numbers correctly (e.g., A1, A3, A8, A10 all go in A1-10)
